...
public class CourseQ3 {
    ...
    public CourseQ3(int courseId, int professorId){
        this(courseId);
        this.professorId = professorId;
    }

    public CourseQ3(int courseId, int professorId, int credits){
    ...
    public void removeDuplicates(int courseId[]){
        List<String> list = new ArrayList<>();
        for(int i =0; i < courseId.length; i++){
            if(list.contains(Integer.toString(courseId[i])))
                continue;
            else
                list.add(Integer.toString(courseId[i]));
        }
    ...
    public static void main(String[] args){
        int[] courseId = new int[]{1,6,2,3,2,4,5,6,7};
        CourseQ3 test = new CourseQ3(courseId[0]);
        test.removeDuplicates(courseId);
